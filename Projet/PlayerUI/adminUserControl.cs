...
namespace PlayerUI
{
    public partial class adminUserControl : UserControl
    {
        ...
        void fill_user()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("select * from ADMIN WHERE idCompte is null ", con);
        ...
        void updateuser(int idu, int idcmpt)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand(" update ADMIN set idCompte=" + idcmpt + " where idAdmin=" + idu + " ", con);
                cmd.ExecuteNonQuery();
            }
        ...
        void ajoutercompte(int idu, string login, string pass)
        {
            try
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("insert into COMPTE values('" + login + "','" + pass + "','ADMIN')", con);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Vous avez associé un compte à l'admin:  " + gunaComboBox2.Text, "Succés", MessageBoxButtons.OK, MessageBoxIcon.Information);
